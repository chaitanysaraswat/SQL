...
SELECT COUNT(*) TOTAL_NO_OF_DIRECTOR_MAPPING_ROWS
FROM DIRECTOR_MAPPING;

SELECT COUNT(*) TOTAL_NO_OF_MOVIE_ROWS
FROM GENRE;

SELECT COUNT(*) TOTAL_NO_OF_MOVIE_ROWS
FROM MOVIE;

SELECT COUNT(*) TOTAL_NO_OF_NAMES_ROWS
FROM NAMES;

SELECT COUNT(*) TOTAL_NO_OF_RATINGS
FROM RATINGS;

SELECT COUNT(*) AS TOTAL_NO_OF_ROLE_MAPPING
FROM ROLE_MAPPING;
...
SELECT YEAR(DATE_PUBLISHED), COUNT(*) AS number_of_movies
FROM MOVIE
GROUP BY YEAR(DATE_PUBLISHED)
order by YEAR(DATE_PUBLISHED) ASC;


SELECT month(DATE_PUBLISHED), COUNT(*) AS number_of_movies
FROM MOVIE
...
GROUP BY COUNTRY
HAVING COUNTRY='USA' OR COUNTRY='INDIA';
...
join movie m
on m.id=g.movie_id
where year(m.date_published)=2017 and month(m.date_published)=3  and m.country   like '%USA%' and r.total_votes>1000
group by genre
...
join genre g
on g.movie_id=r.movie_id
where lower(m.title) like 'The%' and avg_rating>8
...
select r.median_rating,count(r.median_rating)
from movie m
join ratings r
on r.movie_id=m.id
...
select m.country,sum(total_votes)
from movie m
join ratings r
on r.movie_id=m.id
...
select 
sum(case  when country like '%Germany%' then total_votes else 0 end) as german_votes,
sum(case  when country like '%Italy%' then total_votes else 0 end)as italian_votes
from movie m
join ratings r
on r.movie_id=m.id;
...
sum(case when known_for_movies is null then 1 else 0 end) as date_of_birth_nulls
from names;
...
select n.namecount(*) as movie_count
from role_mapping r
join movie m
on r.movie_id=m.id
...
select n.name,count(r.movie_id)
...
# nonaggregated column 'imdb.r.total_votes' which is not functionally dependent on columns 
